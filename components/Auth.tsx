...
const Auth: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isStaffMode) {
        if (
          staffUsername === "AyeshaDurrani" &&
          staffPassword === "AyeshaDurrani244623"
        ) {
          onLogin({
            name: "Administrator",
            role: "admin",
        ...
        const facultyMember = facultyList.find(
          (f) =>
            (f.username === staffUsername || f.name === staffUsername) &&
            f.password === staffPassword
        );
      ...
      if (isRegistering) {
        ...
        try {
          await db.saveUser(newUser, selectedFile || undefined);
          onLogin(newUser);
        } catch (err) {
        ...
        if (foundUser) {
          ...
          setError(
            "Account not found. Please register first or check your email."
          );
        }
      }
    } catch (e) {
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-indigo-900 relative overflow-hidden flex-col justify-between p-12 text-white">
        ...
        <div className="relative z-10 space-y-8 max-w-lg">
          ...
          <p className="text-lg text-indigo-200">
            Access advanced tools like Gemini Chat, Image Analysis, and smart
            visual aids to accelerate your academic journey.
          </p>
                        ...
                        <div className="space-y-1 text-center relative w-full">
                          ...
                          <input
                            ...
                            onChange={(e) => {
                              const file = e.target.files?.[0];
                              if (file) {
                                setSelectedFile(file); // raw File for Supabase
                                const reader = new FileReader();
                                reader.onloadend = () =>
                                  setFeeFile(reader.result as string); // preview
                                reader.readAsDataURL(file);
                              }
                            }}
                          />
                          ...
                          {feeFile ? (
                            <div className="flex flex-col items-center">
                              <img
                                src={feeFile}
                                alt="Fee Preview"
                                className="h-20 w-auto mb-2 rounded shadow-sm border border-gray-200"
                              />
                              <span className="text-green-600 flex items-center text-sm font-medium bg-green-50 px-2 py-1 rounded-full">
